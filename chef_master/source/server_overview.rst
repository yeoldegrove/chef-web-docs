...
.. list-table::
   ...
   * - ``postgresql['db_connection_superuser']``
     - Optional when ``postgresql['external']`` is set to ``true``. The PostgreSQL superuser name in ``'username@hostname'`` format (e.g. ``'opscode_pgsql@my_postgresql.postgres.database.azure.com'``) used for creating PostgreSQL connections, where ``username`` would normally equal the value specified in ``postgresql['db_superuser']``. **Required when using external PostgreSQL on Microsoft Azure**. See ``postgresql['db_superuser']`` below. Default value: ``nil``.
